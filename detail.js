const getDetails = async () => {
  ...
  data.products.map((product) => {
    if (product.Category == Category && product.id != id) {
      ...
      pboxText.setAttribute(
        "href",
        `detail.html?category=${product.Category}&id=${product.id}`
      );
  ...
  if (categoriesProducts) {
    ...
    [...document.querySelectorAll(".category-Prices li")].map((list) => {
      list.getAttribute("min") == "Default" && list.classList.add("active");

      list.addEventListener("click", () => {
        document
          .querySelector(".category-Prices .active")
          .classList.remove("active");
        list.classList.add("active");
        [...document.querySelectorAll(".category-products .product-box")].map(
          (product) => {
            product.classList.add("d-none");
            let element = product.querySelector(
              ".product-prices li:first-of-type"
            );

            if (list.getAttribute("max")) {
              if (
                +element.innerText.replace("$", "") >=
                  list.getAttribute("min") &&
                +element.innerText.replace("$", "") <= list.getAttribute("max")
              ) {
                product.classList.remove("d-none");
              }
            } else {
              if (
                +element.innerText.replace("$", "") >= list.getAttribute("min")
              ) {
                product.classList.remove("d-none");
    ...
    [...document.querySelectorAll(".category-sizes li")].map((list) => {
      list.addEventListener("click", () => {
        document.querySelector(".category-sizes .active") &&
          document
            .querySelector(".category-sizes .active")
            .classList.remove("active");
        list.classList.add("active");
      });
    });
    ...
    document.querySelector(
      ".sub-categoriesUL li:first-of-type"
    ).innerText = `All products (${data.products.length})`;
    document.querySelector(
      ".sub-categoriesUL li:nth-of-type(2)"
    ).innerText = `Men (${
      data.products.filter((a) => a.Category == "Men").length
    })`;
    document.querySelector(
      ".sub-categoriesUL li:nth-of-type(3)"
    ).innerText = `Women (${
      data.products.filter((a) => a.Category == "Women").length
    })`;
    document.querySelector(
      ".sub-categoriesUL li:nth-of-type(4)"
    ).innerText = `Bags (${
      data.products.filter((a) => a.Category == "Bag").length
    })`;
    document.querySelector(
      ".sub-categoriesUL li:nth-of-type(5)"
    ).innerText = `Shoes (${
      data.products.filter((a) => a.Category == "Shoes").length
    })`;
    document.querySelector(
      ".sub-categoriesUL li:last-of-type"
    ).innerText = `Watches (${
      data.products.filter((a) => a.Category == "Watches").length
    })`;

    [...document.querySelectorAll(".sub-categoriesUL li")].map((List) => {
      List.addEventListener("click", (e) => {
        document
          .querySelector(".sub-categoriesUL .active")
          .classList.remove("active");
        List.classList.add("active");
        ...
        window.location.href =
          window.location.href.split("=")[0] + `=${Category}`;
      });
    });
  }
